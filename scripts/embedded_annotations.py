...
which basically adds both opusreader and embedded highlights and bookmarks (if `ADD_BOOKMARKS_TO_OPUSREADER` is True). Alternatively, you can use
...
# if set to true, we re-add the bookmarks into opusreader, so we have both types of bookmarks
ADD_BOOKMARKS_TO_OPUSREADER = False
PATH_TO_OPUSREADER = r'path/to/opusreader.exe'
...
def add_bookmark(doc_path, page_number, location, text):
	...
	doc.saveIncr()
	doc.close()
	if ADD_BOOKMARKS_TO_OPUSREADER:
		subprocess.run([PATH_TO_OPUSREADER, '--execute-command', 'add_bookmark','--execute-command-data', text])
		subprocess.run([PATH_TO_OPUSREADER, '--execute-command', 'reload'])
