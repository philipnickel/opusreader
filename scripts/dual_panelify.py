...
new_command _dual_panelify python /path/to/dual_panelify.py "%{opusreader_path}" "%{file_path}"

now you can execute _dual_panelify command using opusreader's command window.
'''
...
if __name__ == '__main__':
    ...
    for i in range(pdf_reader.numPages // 2):
        if (datetime.datetime.now() - last_update_time).seconds > UPDATE_EVERY_SECONDS:
            last_update_time = datetime.datetime.now()
            subprocess.run([opusreader_path, '--execute-command', 'set_status_string', '--execute-command-data', 'Dual panelifying {} / {}'.format((i+1) * 2, pdf_reader.numPages)])
    ...
    subprocess.run([opusreader_path, '--execute-command', 'set_status_string', '--execute-command-data', 'Writing new file to disk'])
    with open(dual_panel_file_path, 'wb') as f:
        pdf_writer.write(f)

    subprocess.run([opusreader_path, '--execute-command', 'clear_status_string'])
    subprocess.run([opusreader_path, '--new-window', dual_panel_file_path])