...
void MainWidget::execute_command(std::wstring command, std::wstring text, bool wait) {
    ...
    if (command_parts.size() > 0) {
        ...
        for (int i = 0; i < command_parts.size(); i++) {
            ...
            command_parts[i].replace("%{opusreader_path}", QCoreApplication::applicationFilePath());
...
void MainWidget::setup_window_transparency() {
    std::wcout << L"[DEBUG] setup_window_transparency called" << std::endl;
    std::wcout << L"[DEBUG] ENABLE_TRANSPARENCY = " << ENABLE_TRANSPARENCY << std::endl;
    std::wcout << L"[DEBUG] WINDOW_TRANSPARENCY = " << WINDOW_TRANSPARENCY << std::endl;
    std::wcout << L"[DEBUG] MACOS_BLUR_MATERIAL = " << MACOS_BLUR_MATERIAL << std::endl;
    
    if (!ENABLE_TRANSPARENCY) {
        std::wcout << L"[DEBUG] Transparency disabled, returning" << std::endl;
        return;
    }
    ...
    // On macOS, set up NSVisualEffectView to blur content BEHIND the window
    // The Qt content (PDF) will be rendered sharp on top of the blurred background
    // Must be done after window is created
    QTimer::singleShot(100, this, [this]() {
        std::wcout << L"[DEBUG] Timer fired, checking window handle..." << std::endl;
        QWindow* window = windowHandle();
        if (!window) {
            std::wcout << L"[DEBUG] ERROR: No window handle!" << std::endl;
            return;
        }
        std::wcout << L"[DEBUG] Window handle OK, getting NSView..." << std::endl;
        
        void* nsview = reinterpret_cast<void*>(window->winId());
        if (!nsview) {
            std::wcout << L"[DEBUG] ERROR: No NSView!" << std::endl;
            return;
        }
        std::wcout << L"[DEBUG] NSView OK, getting NSWindow..." << std::endl;
        ...
        if (!nswindow) {
            std::wcout << L"[DEBUG] ERROR: No NSWindow!" << std::endl;
            return;
        }
        std::wcout << L"[DEBUG] NSWindow OK, creating NSVisualEffectView..." << std::endl;
        
        // Get the current content view (Qt's view)
        id qtContentView = ((id(*)(id, SEL))objc_msgSend)(
            nswindow,
            sel_registerName("contentView")
        );
        if (!qtContentView) {
            std::wcout << L"[DEBUG] ERROR: No content view!" << std::endl;
            return;
        }
        
        // Get the frame of the current content view
        typedef struct { double x, y, width, height; } CGRect;
        CGRect contentFrame = ((CGRect(*)(id, SEL))objc_msgSend)(
            qtContentView,
            sel_registerName("frame")
        );
        
        // Create NSVisualEffectView
        Class NSVisualEffectViewClass = objc_getClass("NSVisualEffectView");
        if (!NSVisualEffectViewClass) {
            std::wcout << L"[DEBUG] ERROR: NSVisualEffectView class not found!" << std::endl;
            return;
        }
        ...
        // Material options (NSVisualEffectMaterial):
        // 0 = NSVisualEffectMaterialAppearanceBased (adapts to light/dark mode)
        // 1 = NSVisualEffectMaterialLight
        // 2 = NSVisualEffectMaterialDark
        // 3 = NSVisualEffectMaterialTitlebar
        // 4 = NSVisualEffectMaterialSelection
        // 7 = NSVisualEffectMaterialMenu
        // 8 = NSVisualEffectMaterialPopover
        // 9 = NSVisualEffectMaterialSidebar
        // 11 = NSVisualEffectMaterialHeaderView
        // 14 = NSVisualEffectMaterialSheet
        // 15 = NSVisualEffectMaterialWindowBackground
        // 17 = NSVisualEffectMaterialHUDWindow
        // 18 = NSVisualEffectMaterialFullScreenUI
        // 19 = NSVisualEffectMaterialToolTip
        // 21 = NSVisualEffectMaterialContentBackground
        // 23 = NSVisualEffectMaterialUnderWindowBackground (recommended for behind-window blur)
        // 24 = NSVisualEffectMaterialUnderPageBackground
        
        // Use configurable material - default is UnderWindowBackground
        // This material is designed specifically for blurring content behind the window
        ...
        std::wcout << L"[DEBUG] ✓ Window blur effect setup complete!" << std::endl;
    });
#else
...
void MainWidget::setup_native_window() {
#ifdef Q_OS_MACOS
    QWindow* window = windowHandle();
    if (!window) {
        // Window not created yet, try again after show event
        QTimer::singleShot(0, this, [this]() {
            setup_native_window();
        });
        return;
