...
void PdfViewOpenGLWidget::initializeGL() {
	...
	if (!shared_gl_objects.is_initialized) {
		...
		shared_gl_objects.transparency_uniform_location = glGetUniformLocation(shared_gl_objects.rendered_program, "transparency");
	...
	is_helper(is_helper)
{
	...
	// Enable alpha channel for transparency support (for blur effect behind window)
...
void PdfViewOpenGLWidget::render_overview(OverviewState overview) {
	...
	float zoom_level = (view_width / page_width) * overview_zoom;
	...
	float page_width_normalized = 2 * page_width * zoom_level / document_view->get_view_width();
	float page_center_x = (window_rect.x0 + window_rect.x1) / 2;
	float page_min_x = page_center_x - page_width_normalized / 2;
	float page_max_x = page_center_x + page_width_normalized / 2;
...
void PdfViewOpenGLWidget::render(QPainter* painter) {
	...
	if (render_count == 0) {
		std::wcout << L"[DEBUG] render() called: ENABLE_TRANSPARENCY=" << ENABLE_TRANSPARENCY 
		          << L", WINDOW_TRANSPARENCY=" << WINDOW_TRANSPARENCY << std::endl;
	}
	render_count++;
	...
	// Enable blending for transparency support (blur effect)
	if (ENABLE_TRANSPARENCY) {
		glEnable(GL_BLEND);
		glBlendFunc(GL_SRC_ALPHA, GL_ONE_MINUS_SRC_ALPHA);
		if (render_count == 1) {
			std::wcout << L"[DEBUG] GL_BLEND enabled for transparency" << std::endl;
		}
	} else {
		glDisable(GL_BLEND);
	}
	...
	if (!valid_document()) {
		// Use transparent background when transparency is enabled to show blur
		float alpha = ENABLE_TRANSPARENCY ? 0.0f : 1.0f;
	...
	// Use transparent background when transparency is enabled to show blur behind
	float alpha = ENABLE_TRANSPARENCY ? 0.0f : 1.0f;
	
	if (color_mode == ColorPalette::Dark) {
		glClearColor(DARK_MODE_BACKGROUND_COLOR[0], DARK_MODE_BACKGROUND_COLOR[1], DARK_MODE_BACKGROUND_COLOR[2], alpha);
	}
	else if (color_mode == ColorPalette::Custom) {
		glClearColor(CUSTOM_COLOR_MODE_EMPTY_BACKGROUND_COLOR[0], CUSTOM_COLOR_MODE_EMPTY_BACKGROUND_COLOR[1], CUSTOM_COLOR_MODE_EMPTY_BACKGROUND_COLOR[2], alpha);
	}
	else {
		glClearColor(BACKGROUND_COLOR[0], BACKGROUND_COLOR[1], BACKGROUND_COLOR[2], alpha);
	}
	glClear(GL_COLOR_BUFFER_BIT | GL_STENCIL_BUFFER_BIT);
...
void PdfViewOpenGLWidget::set_overview_page(std::optional<OverviewState> overview) {
	bool opening_overview = overview.has_value() && !overview_page.has_value();
	...
	if (opening_overview) {
		const float overview_height_fraction = 1.0f / 3.0f;
		overview_half_height = overview_height_fraction;
		// Ensure the overview window starts centered every time it opens.
		set_overview_offsets(0.0f, 0.0f);
		reset_overview_zoom();
	}
}
...
void PdfViewOpenGLWidget::bind_program() {
	...
	if (color_mode == ColorPalette::Dark) {
		glUseProgram(shared_gl_objects.rendered_dark_program);
		glUniform1f(shared_gl_objects.dark_mode_contrast_uniform_location, DARK_MODE_CONTRAST);
		// Get and set transparency uniform
		...
		// Get and set transparency uniform
		GLint trans_loc = glGetUniformLocation(shared_gl_objects.custom_color_program, "transparency");
		if (trans_loc >= 0) {
			glUniform1f(trans_loc, pdf_transparency);
			if (!logged) {
				std::wcout << L"[DEBUG] Set custom_color transparency uniform to " << pdf_transparency << std::endl;
			}
		} else if (!logged) {
...
DocumentPos PdfViewOpenGLWidget::window_pos_to_overview_pos(NormalizedWindowPos window_pos) {
	...
	float window_width = static_cast<float>(document_view->get_view_width());
	float window_height = static_cast<float>(document_view->get_view_height());
	...
	float zoom_level = (overview_width / page_width) * overview_zoom;
...
float PdfViewOpenGLWidget::get_overview_zoom() const {
	return overview_zoom;
}

void PdfViewOpenGLWidget::reset_overview_zoom() {
	overview_zoom = 1.0f;
}

void PdfViewOpenGLWidget::adjust_overview_zoom(float factor) {
	if (!overview_page.has_value()) {
		return;
	}
	constexpr float kMinZoom = 0.1f;
	constexpr float kMaxZoom = 10.0f;
	overview_zoom = std::clamp(overview_zoom * factor, kMinZoom, kMaxZoom);
	update();
}
...
NormalizedWindowPos PdfViewOpenGLWidget::document_to_overview_pos(DocumentPos pos) {
	...
	if (overview_page) {
		...
		float overview_zoom_level = ((2 * overview_half_width) / target_doc->get_page_width(docpos.page)) * overview_zoom;

		float relative_x = abspos.x * overview_zoom_level;
		float aspect_width = static_cast<float>(document_view->get_view_width());
		float aspect_height = static_cast<float>(document_view->get_view_height());
		if (aspect_width <= 0.0f || aspect_height <= 0.0f) {
			return { overview_offset_x, overview_offset_y };
		}
		float aspect = aspect_width / aspect_height;
