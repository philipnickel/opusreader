...
std::string LOG_FILE_NAME = "opusreader_log.txt";
...
void configure_paths(){
...
#ifdef Q_OS_MACOS
	// DISABLED: Don't load from ~/.config/opusreader during development
	// Path mac_home_path(QDir::homePath().toStdWString());
	// Path mac_standard_config_path = mac_home_path.slash(L".config").slash(L"opusreader");
	...
	Path standard_data_path = home_path.slash(L".local").slash(L"share").slash(L"opusreader");
	Path standard_config_path = Path(L"/etc/opusreader");
	...
	Path linux_standard_config_path = linux_home_path.slash(L".config").slash(L"opusreader");
	//user_keys_paths.push_back(mac_standard_config_path.slash(L"keys_user.config"));
	//user_config_paths.push_back(mac_standard_config_path.slash(L"prefs_user.config"));
...
int main(int argc, char* args[]) {
	...
	if (has_arg(argc, args, "--version")) {
		std::cout << "opusreader " << APPLICATION_VERSION << "\n";
		return 0;
	}
