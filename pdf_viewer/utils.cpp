...
void install_app(const char *argv0)
{
	...
	OPEN_KEY(classes, "OpusReader", testpdf);
	...
	SET_KEY(open, "FriendlyAppName", "OpusReader");
	SET_KEY(command, "", buf);
	SET_KEY(supported_types, ".pdf", "");
	SET_KEY(pdf_progids, "opusreader", "");
...
QCommandLineParser* get_command_line_parser() {
	...
	parser->setApplicationDescription("OpusReader is a PDF reader designed for reading research papers and technical books.");
	//parser->addVersionOption();

	//QCommandLineOption reuse_instance_option("reuse-instance");
	//reuse_instance_option.setDescription("When opening a new file, reuse the previous instance of opusreader instead of opening a new window.");
	//parser->addOption(reuse_instance_option);

	//QCommandLineOption new_instance_option("new-instance");
	//new_instance_option.setDescription("When opening a new file, create a new instance of opusreader.");
	//parser->addOption(new_instance_option);

	QCommandLineOption new_window_option("new-window");
	new_window_option.setDescription("Open the file in a new window but within the same opusreader instance (reuses the previous window if a opusreader window with the same file already exists).");
	parser->addOption(new_window_option);

	QCommandLineOption reuse_window_option("reuse-window");
	reuse_window_option.setDescription("Force opusreader to reuse the current window even when should_launch_new_window is set.");
	parser->addOption(reuse_window_option);

	QCommandLineOption nofocus_option("nofocus");
	nofocus_option.setDescription("Do not bring the opusreader instance to foreground.");
	parser->addOption(nofocus_option);

	QCommandLineOption version_option("version");
	version_option.setDescription("Print opusreader version.");
 ...
 In <command>, %1 is filled with the file name and %2 is filled with the line number.", "command");
	...
	QCommandLineOption command_option("execute-command", "The command to execute on running instance of opusreader", "execute-command");
